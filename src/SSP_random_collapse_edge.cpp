  ...
  Eigen::VectorXi & FIdx_onering_pre)
{
  ...
  {
    E(e,0) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    E(e,1) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    EF(e,0) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    EF(e,1) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    EI(e,0) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    EI(e,1) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
  };
  ...
  for(int side = 0;side<2;side++)
  {
    ...
    kill_edge(e1);
    // Kill f
    F(f,0) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    F(f,1) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
    F(f,2) = IGL_RANDOM_COLLAPSE_EDGE_NULL;
  ...
  std::vector<std::vector<int>> & decIM)
{
  ...
  std::tuple<double,int,int> p, p_tmp;
  while(true)
  {
    // Check if Q is empty
    if(Q.empty())
    ...
    // pop from Q
    int nth_edge = 1 + (rand() % 100);  // choose a random nth item from Q, where 1<n<100
    if (nth_edge > Q.size())
      nth_edge = Q.size();
    // remove the first (n-1) elements from the Q
    std::list< std::tuple<double,int,int> > edge_holder;
    for (int ii=0; ii<nth_edge; ii++)
    {
      p_tmp = Q.top();
      edge_holder.push_front(p_tmp);
      Q.pop();
    }
    // pick the nth element
    p = Q.top();
    if(std::get<0>(p) == std::numeric_limits<double>::infinity())
      return false; // edge is infinite cost
    Q.pop();
    // put back elements from the "edge_holder"
    while (edge_holder.size() > 0)
    {
      p_tmp = edge_holder.front();
      Q.push(p_tmp);
      edge_holder.pop_front();
    }
    e = std::get<1>(p);
  ...
  {
    collapsed = SSP_random_collapse_edge(
      e,C.row(e),
      Nsv,Nsf,Ndv,Ndf,
  ...
  if(collapsed)
  {
    ...
    for(auto & n : Nf)
    {
      if(F(n,0) != IGL_RANDOM_COLLAPSE_EDGE_NULL ||
          F(n,1) != IGL_RANDOM_COLLAPSE_EDGE_NULL ||
          F(n,2) != IGL_RANDOM_COLLAPSE_EDGE_NULL)
      {
        for(int v = 0;v<3;v++)
        {
          // get edge id
